...
// Sends the team invitation email for exactly one recipient.
// Authorization mirrors the previous pipeline: either an internal service-role
// caller, or a signed-in ADMIN of the organization that owns a pending
// invitation for that recipient.
...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !serviceKey) {
    console.error('send-invitation-email: missing env')
  ...
  const inviteLink = String(body.inviteLink ?? '')
  ...
  if (!inviteLink) return json({ error: 'inviteLink is required' }, 400)
  ...
  try {
    const result = await sendTemplateEmail(TEMPLATE, recipientEmail, {
      idempotencyKey,
      templateData: { inviteLink, role, department },
    })
    ...
    console.error('send-invitation-email: send failed', { message })
    await logEmailSend(supabase, {
      template_name: TEMPLATE,
      recipient_email: recipientEmail,
      status: 'failed',
