-- Phase: single transaction ownership for procurement lifecycle
-- Every PR has at most one row in public.transactions (already enforced by transactions.pr_id UNIQUE).
-- This migration makes child records carry that transaction FK and rewires workflow functions
-- to resolve/update the existing transaction instead of treating invoices/quotes/payments separately.

-- 1) Child-record transaction links -------------------------------------------------
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'quote_requests_transaction_id_fkey') THEN
    ALTER TABLE public.quote_requests
      ADD CONSTRAINT quote_requests_transaction_id_fkey
      FOREIGN KEY (transaction_id) REFERENCES public.transactions(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'quotes_transaction_id_fkey') THEN
    ALTER TABLE public.quotes
      ADD CONSTRAINT quotes_transaction_id_fkey
      FOREIGN KEY (transaction_id) REFERENCES public.transactions(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'invoices_transaction_id_fkey') THEN
    ALTER TABLE public.invoices
      ADD CONSTRAINT invoices_transaction_id_fkey
      FOREIGN KEY (transaction_id) REFERENCES public.transactions(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'payment_audit_log_transaction_id_fkey') THEN
    ALTER TABLE public.payment_audit_log
      ADD CONSTRAINT payment_audit_log_transaction_id_fkey
      FOREIGN KEY (transaction_id) REFERENCES public.transactions(id) ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'receipts_transaction_id_fkey') THEN
    ALTER TABLE public.receipts
...
CREATE INDEX IF NOT EXISTS idx_quote_requests_transaction_id ON public.quote_requests(transaction_id);
CREATE INDEX IF NOT EXISTS idx_quotes_transaction_id ON public.quotes(transaction_id);
CREATE INDEX IF NOT EXISTS idx_invoices_transaction_id ON public.invoices(transaction_id);
CREATE INDEX IF NOT EXISTS idx_payment_audit_log_transaction_id ON public.payment_audit_log(transaction_id);
CREATE INDEX IF NOT EXISTS idx_receipts_transaction_id ON public.receipts(transaction_id);
CREATE INDEX IF NOT EXISTS idx_payment_allocations_invoice_transaction ON public.payment_allocations(invoice_id, transaction_id);

-- 2) Central resolver: one transaction per PR --------------------------------------
...
BEGIN
  ...
  INSERT INTO public.transactions (
    ...
    CASE WHEN _mark_invoiced THEN 'INVOICED' ELSE 'APPROVED_NOT_PAID' END,
    now(),
    _invoice_id,
    _document_url,
  ...
  ON CONFLICT (pr_id) DO UPDATE
    SET organization_id = EXCLUDED.organization_id,
        ...
        status = CASE
          WHEN _mark_invoiced AND public.transactions.status = 'APPROVED_NOT_PAID' THEN 'INVOICED'
...
REVOKE EXECUTE ON FUNCTION public.ensure_transaction_for_pr(uuid, uuid, text, numeric, text, uuid, boolean) FROM anon, authenticated, public;

-- 3) Hydration triggers for children ----------------------------------------------
...
BEGIN
  IF NEW.transaction_id IS NULL THEN
    NEW.transaction_id := public.ensure_transaction_for_pr(NEW.pr_id, NEW.supplier_id, NULL, NULL, NULL, NULL, false);
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  IF NEW.transaction_id IS NULL THEN
    NEW.transaction_id := public.ensure_transaction_for_pr(NEW.pr_id, NEW.supplier_id, _supplier_name, NEW.amount, NEW.document_url, NULL, false);
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  ...
  NEW.transaction_id := COALESCE(
    NEW.transaction_id,
    public.ensure_transaction_for_pr(NEW.pr_id, NEW.supplier_id, _supplier_name, _quote_amount, NEW.document_url, NEW.id, true)
  );
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION public.tg_payment_allocation_attach_transaction()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  _txn_id uuid;
BEGIN
  IF NEW.transaction_id IS NULL AND NEW.invoice_id IS NOT NULL THEN
    SELECT COALESCE(i.transaction_id, t.id)
      INTO _txn_id
...
CREATE OR REPLACE FUNCTION public.tg_receipt_attach_transaction()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  IF NEW.transaction_id IS NULL AND NEW.ocr_analysis_id IS NOT NULL THEN
    SELECT COALESCE(i.transaction_id, t.id)
      INTO NEW.transaction_id
    FROM public.ocr_analyses oa
    LEFT JOIN public.invoices i ON i.id = oa.invoice_id
    LEFT JOIN public.transactions t ON t.pr_id = oa.pr_id
    WHERE oa.id = NEW.ocr_analysis_id;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_quote_request_attach_transaction ON public.quote_requests;
CREATE TRIGGER trg_quote_request_attach_transaction
  BEFORE INSERT OR UPDATE OF pr_id, supplier_id, transaction_id ON public.quote_requests
  FOR EACH ROW EXECUTE FUNCTION public.tg_quote_request_attach_transaction();

DROP TRIGGER IF EXISTS trg_quote_attach_transaction ON public.quotes;
CREATE TRIGGER trg_quote_attach_transaction
  BEFORE INSERT OR UPDATE OF pr_id, supplier_id, transaction_id, amount, document_url ON public.quotes
  FOR EACH ROW EXECUTE FUNCTION public.tg_quote_attach_transaction();

DROP TRIGGER IF EXISTS trg_invoice_attach_transaction ON public.invoices;
CREATE TRIGGER trg_invoice_attach_transaction
  BEFORE INSERT OR UPDATE OF pr_id, supplier_id, quote_id, transaction_id, document_url ON public.invoices
  FOR EACH ROW EXECUTE FUNCTION public.tg_invoice_attach_transaction();

DROP TRIGGER IF EXISTS trg_payment_allocation_attach_transaction ON public.payment_allocations;
CREATE TRIGGER trg_payment_allocation_attach_transaction
  BEFORE INSERT OR UPDATE OF invoice_id, transaction_id ON public.payment_allocations
  FOR EACH ROW EXECUTE FUNCTION public.tg_payment_allocation_attach_transaction();

DROP TRIGGER IF EXISTS trg_receipt_attach_transaction ON public.receipts;
CREATE TRIGGER trg_receipt_attach_transaction
  BEFORE INSERT OR UPDATE OF ocr_analysis_id, transaction_id ON public.receipts
  FOR EACH ROW EXECUTE FUNCTION public.tg_receipt_attach_transaction();

REVOKE EXECUTE ON FUNCTION public.tg_quote_request_attach_transaction() FROM anon, authenticated, public;
REVOKE EXECUTE ON FUNCTION public.tg_quote_attach_transaction() FROM anon, authenticated, public;
REVOKE EXECUTE ON FUNCTION public.tg_invoice_attach_transaction() FROM anon, authenticated, public;
REVOKE EXECUTE ON FUNCTION public.tg_payment_allocation_attach_transaction() FROM anon, authenticated, public;
REVOKE EXECUTE ON FUNCTION public.tg_receipt_attach_transaction() FROM anon, authenticated, public;

-- 4) Finance approval updates/creates exactly one transaction ---------------------
...
BEGIN
  IF (TG_OP = 'UPDATE' AND NEW.status::text = 'FINANCE_APPROVED' AND OLD.status::text IS DISTINCT FROM 'FINANCE_APPROVED')
    ...
    _txn_id := public.ensure_transaction_for_pr(NEW.id, NULL, NULL, NEW.total_amount, NEW.document_url, NULL, false);
...
-- 5) Invoice folding only updates the existing PR transaction ---------------------
...
-- 6) Quote acceptance binds accepted quote/supplier to the one transaction ---------
...
-- 7) Batch creation/editing/confirmation: payments are transaction-owned ----------
...
BEGIN
  ...
  LOOP
    ...
    IF _invoice_id IS NOT NULL THEN
      ...
      UPDATE public.transactions
      SET status = CASE WHEN status = 'APPROVED_NOT_PAID' THEN 'INVOICED' ELSE status END,
          updated_at = now()
      WHERE id = _txn_id;
  ...
  IF jsonb_typeof(_add) = 'array' THEN
    ...
    LOOP
      ...
      IF _invoice_id IS NOT NULL THEN
        ...
        END IF;
        IF EXISTS (SELECT 1 FROM public.payment_allocations WHERE transaction_id = _txn_id AND batch_id <> _batch_id) THEN
          RAISE EXCEPTION 'Transaction is already linked to another payment batch';
        END IF;
...
BEGIN
  ...
  -- All invoice payments resolve to the same transaction as the invoice.
  FOR _alloc IN
  ...
  LOOP
    ...
    SET amount_paid = _paid_total,
        status = CASE WHEN _new_status = 'PAID' THEN 'FULLY_PAID' ELSE 'PARTIALLY_PAID' END,
  ...
  -- Reimbursement allocations remain reimbursement-owned.
  FOR _alloc IN
  ...
  -- Direct transaction allocations.
  FOR _alloc IN
  ...
  LOOP
    ...
    IF _paid_total >= _alloc.target_amount THEN _new_status := 'FULLY_PAID'; ELSE _new_status := 'PARTIALLY_PAID'; END IF;
...
-- 8) Timeline/payment logging should always resolve transaction_id ----------------
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    PERFORM public.log_transaction_event(NEW.pr_id, _txn, NEW.organization_id, 'INVOICE_UPLOADED', 'Invoice Uploaded', NULL, NEW.status, NULL, NEW.status);
  ELSIF TG_OP = 'UPDATE' AND NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    END IF;
    PERFORM public.log_transaction_event(NEW.pr_id, _txn, NEW.organization_id, _type, _title, NULL, NEW.status, OLD.status, NEW.status);
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  _txn := COALESCE(NEW.transaction_id, (SELECT id FROM public.transactions WHERE pr_id = NEW.pr_id LIMIT 1));
  IF TG_OP = 'INSERT' THEN
    PERFORM public.log_transaction_event(NEW.pr_id, _txn, NEW.organization_id, 'QUOTE_UPLOADED', 'Quote Uploaded', NULL, NEW.status, NULL, NEW.status);
  ELSIF TG_OP = 'UPDATE' AND NEW.status IS DISTINCT FROM OLD.status THEN
...
BEGIN
  ...
  IF NEW.invoice_id IS NOT NULL THEN
    SELECT pr_id, transaction_id INTO _pr, _txn FROM public.invoices WHERE id = NEW.invoice_id;
  END IF;
